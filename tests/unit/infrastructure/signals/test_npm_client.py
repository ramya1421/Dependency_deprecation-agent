...
def _dependency(
    name: str = "request",
    resolved_version: str | None = "2.88.0",
    ecosystem: Ecosystem = Ecosystem.JAVASCRIPT,
) -> Dependency:
    return Dependency(
        name=name,
        ecosystem=ecosystem,
...
async def test_offline_cache_miss_returns_empty_list(connection: sqlite3.Connection) -> None:
    ...
    client = NpmClient(offline_client)
...
@respx.mock
async def test_non_javascript_dependency_makes_no_network_call(
    connection: sqlite3.Connection,
) -> None:
    route = respx.get("https://registry.npmjs.org/request").mock(
        return_value=httpx.Response(200, json={"dist-tags": {}, "versions": {}, "time": {}})
    )
    client = NpmClient(_http_client(connection))

    signals = await client.fetch(_dependency(ecosystem=Ecosystem.PYTHON))
...
async def test_major_versions_behind_produces_outdated_signal(
    ...
    assert signals[0].payload["major_versions_behind"] == 4
